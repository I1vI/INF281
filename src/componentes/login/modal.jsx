...
const Modal = ({ isOpen, onClose }) => {
    ...
    const onSubmit = async (data) => {
        try {
            localStorage.setItem("email", data.email); //Guardamos el email para la verificacion
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block font-medium">
                                País {errors.pais && <span className="text-red-500 text-sm">(campo requerido)</span>}
                            </label>
                            <CountrySelect
                                onChange={(e) => {
                                    setidPais(e.id);
                        ...
                        <div>
                            <label className="block font-medium">
                                Ciudad {errors.ciudad && <span className="text-red-500 text-sm">(campo requerido)</span>}
                            </label>
                            <StateSelect
                                disabled={!idPais}
                                countryid={idPais}
