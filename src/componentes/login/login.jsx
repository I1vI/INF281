    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Si el backend responde con un token, guárdalo en localStorage
                ...
                // Redirigir a la página de inicio (ajústala según tu aplicación)
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen">
            <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" action="POST">
                    <div className="flex items-center border border-gray-300 p-3 rounded-md bg-gray-100">
                        <FaEnvelope className="text-gray-600" />
                        <input type="email" placeholder="Dirección Email" {...register('email', { required: true })} 
                            className="bg-transparent border-none outline-none w-full text-gray-700 ml-2" />
                    </div>
                    {errors.email && <p className="text-red-500 text-sm">El campo es requerido</p>}
                    
                    <div className="flex items-center border border-gray-300 p-3 rounded-md bg-gray-100 relative">
                        <FaLock className="text-gray-600" />
                        <input type={visible ? "text" : "password"} placeholder="Contraseña" {...register('contrasenia', { required: true })} 
                            className="bg-transparent border-none outline-none w-full text-gray-700 ml-2" />
                        <button type="button" onClick={() => setVisible(!visible)} className="absolute right-3 text-gray-600 cursor-pointer">
                            {visible ? <AiFillEye /> : <AiFillEyeInvisible />} 
                        </button>
                    </div>
                    {errors.contrasenia && <p className="text-red-500 text-sm">El campo es requerido</p>}
                    ...
                    <div className="text-center text-sm text-gray-600">
                        ¿Olvidaste tu contraseña? <Link to="/recuperar" className="text-blue-600 cursor-pointer hover:text-purple-600">Haz clic aquí </Link>
                    </div>
                    <div className="text-center text-sm text-gray-600">
                    ...
                    <button type="submit" className="w-full bg-orange-500 text-white p-3 rounded-md hover:bg-orange-600 transition">Iniciar Sesión</button>
                </form>
            </div>
        </div>
    );
